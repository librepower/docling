...
show_help() {
    ...
    echo "  --wheels-only   Install pre-built optimized wheels only"
    ...
    echo ""
    echo "Power Architecture Options (AIX):"
    echo "  --power9        Use POWER9 optimized wheels (default, P9/P10/P11 compatible)"
    echo "  --power10       Use POWER10 optimized wheels (MMA enabled, P10/P11 only)"
    echo ""
    echo "Examples:"
    echo "  $0              # Full installation (auto-detects processor)"
    echo "  $0 --power10    # Install with POWER10 optimizations"
    echo "  $0 --verify     # Check if everything works"
    echo "  $0 --demo       # Run the RAG demo"
    echo ""
    echo "Supported Platforms:"
    echo "  - AIX 7.3+ on IBM Power (POWER9, POWER10, POWER11)"
    echo "  - Ubuntu 22.04+ ppc64le"
    echo ""
